  ...
  <div class="fund-flow">
    ...
    <div class="charts-container">
      ...
      <div class="chart-section">
        <h3 style="text-align: center;color: aliceblue;">行业版块 - Top 5 和 Tail 5</h3>
...
export default {
  ...
  methods: {
    async fetchFundFlowData() {
      try {
        const response = await api.get("/sector-fund-flow", { params: { sector_type: "行业资金流" } });
    ...
    initCharts(data) {
      ...
      this.conceptChartOption = this.createChartOption(conceptData, conceptNames, '行业版块');
    },
    handleChartClick(params) {
      if (params && params.name) {
        const industryName = params.name.split(' - ')[1]; 
        this.$emit('sector-selected', '行业资金流', industryName);
      }
    },
