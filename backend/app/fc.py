...
# 定义过滤函数
def filter_sensitive_words(text, max_attempts=10):
    """
    过滤敏感词，如果包含敏感词则重新生成。
    :param text: 待检查的文本
    ...
    """
    attempts = 0
    while any(word in text for word in SENSITIVE_WORDS):
        if attempts >= max_attempts:
            return "数据生成失败，敏感词超出最大尝试次数"
        text = faker.sentence(nb_words=5)  # 默认重新生成句子
        attempts += 1
    return text
...
def generate_data(column_comment, data_type):
    ...
    for keyword, generator in RULE_MAPPING.items():
        if keyword in column_comment:
            # 生成数据并过滤敏感词
            return filter_sensitive_words(generator())
    
    # 默认处理
    if data_type in ["varchar", "text"]:
        return filter_sensitive_words(faker.sentence(nb_words=5))
    else:
        return filter_sensitive_words(faker.word())
...
# 从数据库中获取所有表名
def fetch_table_names_from_db(host, user, password, database):
    """
    从数据库中获取所有表名。
    :param host: 数据库主机
    :param user: 数据库用户名
...
# 根据前缀筛选表并生成测试数据
def generate_test_data_by_prefix(prefix, host, user, password, database):
    """
    根据前缀筛选表，为每张表逐条生成和插入测试数据。
    如果某条数据插入失败，继续插入下一条。
    :param prefix: 表名前缀
    :param host: 数据库主机
    :param user: 数据库用户名
    ...
    """
    # 获取所有表名
    all_tables = fetch_table_names_from_db(host, user, password, database)
    matched_tables = [table for table in all_tables if table.startswith(prefix)]
    
    if not matched_tables:
        print(f"未找到匹配前缀 '{prefix}' 的数据表。")
        return
    ...
    for table in matched_tables:
        ...
        """
        cursor.execute(query)
        schema = cursor.fetchall()  # 返回包含元组的列表
        ...
        for _ in range(row_count):
            ...
            for column in schema:
                ...
                column_comment = column[1]  # COLUMN_COMMENT
                data_type = column[2]  # DATA_TYPE
                row[column_name] = generate_data(column_comment, data_type)
            ...
            try:
                ...
                connection.commit()
                print(f"表 {table} 成功插入数据：{row}")
            except Exception as e:
                print(f"表 {table} 插入数据失败：{row}, 错误：{e}")
...
"""
"""捞取数据表的所有字段的注释并生成分词"""
def get_column_comments(host, user, password, database):
    """
    连接数据库并获取所有表的列注释
    """
    try:
        ...
        for column in columns:
            if column["COLUMN_COMMENT"]:  # 仅处理有注释的列
                comments.append(column["COLUMN_COMMENT"])
...
def extract_keywords_as_list(comments):
    """
    对列注释分词，并提取核心关键词（如编号、姓名等名词），以 list 返回
    """
    keyword_list = []
    for comment in comments:
        # 使用 jieba 进行分词并标注词性
        words = pseg.lcut(comment)
        keywords = [word for word, flag in words if flag.startswith("n")]  # 筛选名词
        keyword_list.extend(keywords)  # 将关键词加入列表
...
if __name__ == "__main__":
    test_data = generate_test_data_by_prefix("wjqgajjdcjylcsj",DB_CONFIG["host"], DB_CONFIG["user"], DB_CONFIG["password"], DB_CONFIG["database"])
    # test_data = get_column_comments(DB_CONFIG["host"], DB_CONFIG["user"], DB_CONFIG["password"], DB_CONFIG["database"])
    print(test_data)